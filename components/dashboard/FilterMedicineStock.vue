<template>
    <div
        ...
        <div class="flex justify-between">
            ...
            <div>
                <label class="block text-sm font-medium mb-1" for="placeholder">Stock greater than</label>
        ...
        <div>
            ...
            rounded-sm border border-gray-200 p-2 ">
                <div class="underline text-blue-500"><a :href="`/medicine/${medicine?.id}/`">{{ medicine?.brand_name
}}</a>
                </div>
                <div>
...
onMounted(async () => {
    var response = await useBaseFetch(`/admin-api/meds/medicine/?quantity__gte=0`)
...
watch(quantity, async (newQuantity) => {
    ...
    var response = await useBaseFetch(`/admin-api/meds/medicine/?quantity__gte=${newQuantity}`)
