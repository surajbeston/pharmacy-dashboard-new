  ...
  >
    ...
    <div>
      <Dropdown
        ...
        nameAttribute="name"
        valueAttribute="id"
        @selectedObject="filterByManufacturer"
      ></Dropdown>
    </div>
    <div class="relative">
      <button
      ...
      >
        <div
          v-for="option in dateSelectOptions"
          :key="option"
        ...
        >
          ...
          >
            <svg
              ...
              :class="{ invisible: !(selectedOption == option) }"
              width="12"
              height="9"
              viewBox="0 0 12 9"
            >
...
const {results: manufacturers} = await useBaseFetch("/admin-api/meds/manufacturer/?limit=1000&offset=0")
const showOptions = ref(false)

const dateFilterString = ref('')
const manufacturerFilterString = ref('')
...
async function filterByManufacturer(manufacturerId){
    manufacturerFilterString.value = `manufacturer=${manufacturerId}`
}

watch(() => manufacturerFilterString.value + dateFilterString.value, () => {
    console.log(manufacturerFilterString.value)
    console.log(dateFilterString.value)

    emit("filterUrl", `/admin-api/meds/supplier/?${manufacturerFilterString.value}&${dateFilterString.value}`)
})
